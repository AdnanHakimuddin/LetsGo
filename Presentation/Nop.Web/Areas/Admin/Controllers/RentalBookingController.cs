...
{
    public partial class RentalBookingController : BaseAdminController
    {
        #region Fields
        ...
        public RentalBookingController(IOrderModelFactory orderModelfactory,
        ...
        public virtual async Task<IActionResult> List(RentalBookingSearchModel searchModel)
        {
            ...
            //prepare model
            var model = await _orderModelfactory.PrepareRentaBookingListModelAsync(searchModel);
        ...
        public virtual async Task<IActionResult> Create()
        {
            ...
            //prepare model
            var model = await _orderModelfactory.PrepareRentaBookingModelAsync(new RentalBookingModel(), null);
        ...
        public virtual async Task<IActionResult> Create(RentalBookingModel model, bool continueEditing)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                rentalBooking.DateOfDeparture = DateTime.UtcNow;
                rentalBooking.ProgrammeDate = DateTime.UtcNow;
                rentalBooking.BookingDate = (DateTime)_dateTimeHelper.ConvertToUtcTime(model.BookingDate);
                rentalBooking.ReturnDate = (DateTime)_dateTimeHelper.ConvertToUtcTime(model.ReturnDate);
                rentalBooking.CreatedById = _workContext.GetCurrentCustomerAsync().Id;
                rentalBooking.BookingTypeId = (int)BookingEnum.RentalBooking;
                await _orderService.InsertRentalBookingAsync(rentalBooking);

                _notificationService.SuccessNotification(await _localizationService.GetResourceAsync("Admin.Catalog.RentalBooking.Created"));
        ...
        public virtual async Task<IActionResult> Edit(RentalBookingModel model, bool continueEditing)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                _notificationService.SuccessNotification("Rental Booking Updated");
        ...
        public virtual async Task<IActionResult> Delete(int id)
        {
            ...
            _notificationService.SuccessNotification(await _localizationService.GetResourceAsync("Admin.Catalog.RentalBooking.Deleted"));
        ...
        [HttpPost, ActionName("ExportRentalBookingPdf")]
        [FormValueRequired("exportpdf")]
        public virtual async Task<IActionResult> ExportRentalBookingPdf(RentalBookingModel searchModel)
        {
            if (!await _permissionService.AuthorizeAsync(StandardPermissionProvider.ManageCategories))
                return AccessDeniedView();
            ...
            var html = await RenderPartialViewToStringAsync("_RentalBookingPdf", model);
